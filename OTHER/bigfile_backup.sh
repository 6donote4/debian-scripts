...
ARGS=( "$@" )
	...
	case "$1" in
	    -s)
            find $2 -type f -print0 |xargs -P 30 -n 100 -I % -0 rsync --progress  -avuzPR --delete % $3
	    	echo "done"
	    	exit 0
		;;
 		-h|--help)
		...
		;;
	    -c)
            find $2/ -type f -print0 |xargs -P 30 -n 100 -I % -0 rsync --progress -avuzPR  % $3
	    	echo "done"
	    	exit 0
		;;
	    -r)
            find $2 -type f -print0 |xargs -P 30 -n 100 -I % -0 rsync --progress -avze 'ssh -p 2000' $2 $3
	    	echo "done"
	    	exit 0
		;;
		--version)
