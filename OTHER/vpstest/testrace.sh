...
# Revision: 6donote4
# Description: comment some function
#======================================
${Font_suffix}"
...
install(){
	[[ ! -d ${TPATH}/besttrace ]] && wget -O besttrace.zip https://cdn.ipip.net/17mon/besttrace4linux.zip && unzip besttrace.zip -d besttrace && rm besttrace.zip
	[[ ! -d ${TPATH}/besttrace ]] && echo -e "${Error} download failed, please check!" && exit 1
	chmod -R +x ${TPATH}
}
...
test_single(){
	...
	./besttrace -q 1 ${ip} | tee -a -i ${TPATH}/testrace.log 2>/dev/null
...
read -p "输入数字以选择:" function
	...
	if [[ "${function}" == "1" ]]; then
		...
		test_all | tee -a -i ${TPATH}/testrace.log 2>/dev/null
	else
		test_single
	fi
